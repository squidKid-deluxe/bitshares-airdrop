...
# GLOBAL CONSTANTS
PREFIX = "BTS"  # issuing_chain()["prefix"]
ID = (  # issuing_chain()["id"]
...
def wss_handshake(nodeses=None):
...
def wss_query(params, nodes=None):
        ...
        operation = [
            ...
            OrderedDict(
                [
                    ...
                    ("amount", amount_dict),
                    # ("memo", transfer["memo"].encode("ascii")), # FIXME memos are not yet implemented
                    ("extensions", []),
                ]
            ),
        ]
...
def serialize_transaction(tx):
    ...
    # buf += bytes(varint(0))  # effectively varint(0)
...
def prototype_order(_=None):
    ...
    proto = {}
    proto["op"] = ""
    proto["nodes"] = bitshares_nodes()
    proto["header"] = {
        "asset_id": "1.3.6301",
        "currency_id": "1.3.6301",
        "asset_precision": 8,
        "currency_precision": 8,
        "account_id": "",
        "account_name": "",
...
def log_in():
    ...
    if not wif:
        # some random wif
        wif = "5JLw5dgQAx6rhZrHmm6C2ds1V47RweGshynFSWFbaMohsYsBvE8"
